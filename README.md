...
- **IPv6 双栈检测**：检测用户 IPv4/IPv6 连接状态，显示双栈支持情况、各协议 IP 地址及归属地、浏览器协议偏好
- **NAT 类型检测**：通过 WebRTC ICE 候选地址分析判断 NAT 类型（开放/完全锥形/受限锥形/对称型），显示公网映射 IP 和端口一致性
- **HTTP/浏览器指纹**：服务端返回 HTTP 请求头（调试反代配置），前端采集 Canvas 哈希、WebGL 渲染器、时区、语言等浏览器指纹
- **DNS 基准测试**：对比 9 大公共 DNS 解析器（Google/Cloudflare/Quad9/OpenDNS/DNSPod/AliDNS/114DNS）的响应速度，SSE 实时推送结果并排序
- **Web 前端**：内置单页应用，实时展示客户端 IP 与 DNS 解析器 IP 及归属地，含路由追踪、网速测试、IPv6 检测、原理说明、流程图、探测进度条
